...
class FileType extends Component<Props, State> {
  ...
  onChildChekBoxClicked = (e, parentId, id) => {
    ...
    // console.log("[onChildChekBoxClicked]:", updateCheckBoxStatus);
    // updateCheckBoxStatus.map();

    this.setState({ fileTypes: updateCheckBoxStatus });

    // //mehtod 2
    // const fileTypesTemp: Array<FileObject> = [...this.state.fileTypes];
    // console.log("[fileTypesTemp]", fileTypesTemp);

    // let file = fileTypesTemp.find((file) => file.id === parentId);
    // if (file) {
    //   let type = file?.types.find((type) => type.id === id);
    //   if (type) {
    //     type["isChecked"] = e.target.checked;
    //   }
    // }
